...
const setup = () => {
    ...
    const { cols } = calculateGridDimensions(g.cardCount);
    g.gameBoard.style.gridTemplateColumns = `repeat(${cols}, 100px)`;

    const availableContent = Array.from({ length: g.MAX_IMAGES }, (_, i) => `image${i}.jpg`);
    ...
    CardsSet.forEach((content) => {
        const Card = document.createElement('div');
        Card.classList.add('card');
        Card.dataset.image = g.IMAGE_FOLDER + content;
...
const processCardGroup = () => {
    ...
    if (allMatch) {
        g.selectedCards.forEach(card => card.classList.add('matched'));
        setTimeout(() => {
            g.selectedCards = [];
            g.processing = false;
            checkFinish();
        ...
        g.selectedCards.forEach(card => card.classList.add('wrong-match'));
        setTimeout(() => {
